...
interface AddProductDialogProps {
  ...
  onSubmit: (product: Omit<MarketplaceProduct, 'id' | 'sellerId' | 'sellerName' | 'sellerType' | 'createdAt'>) => void;
}
...
export function AddProductDialog({ open, onOpenChange, onSubmit }: AddProductDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-xl max-w-md max-h-[90vh] overflow-y-auto" aria-describedby="add-product-description">
        <DialogHeader>
          <DialogTitle className="flex items-center">
            <Package className="h-5 w-5 mr-2 text-primary" />
            Publier sur la Marketplace
          </DialogTitle>
          <p id="add-product-description" className="sr-only">Formulaire d'ajout d'un nouveau produit au marketplace</p>
          <DialogDescription>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              {productType === 'raw_material' 
                ? 'Prix auquel vous achetez cette matière aux collecteurs/citoyens' 
                : 'Prix auquel vous vendez ce produit fini'}
            </p>
          </div>
